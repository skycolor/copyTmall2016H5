...
function(A) {
	...
	return g.e = function(A, t) {
		...
		if (void 0 !== C[A]) C[A].push(t);
			...
			I.type = "text/javascript", I.charset = "utf-8", I.async = !0, I.src = g.p + "" + ({
				0: "wdata",
				2: "cdata"
			}[A] || A) + ".js?a42321cbe53ac3d3ed68", e.appendChild(I)
		}
	}, g.m = A, g.c = e, g.p = "static/", g(0)
}({
	0: function(A, g, t) {
		(function(A, g) {
		...
		(function(I) {
			...
			function(i) {
				...
				var p = function(A, g, t, e, C) {
						if (A._events = d(E, A._events || {}, g, t, {
							context: e,
							ctx: A,
							listening: C
						}), C) {
				...
				l.off = function(A, g, t) {
					return this._events ? (this._events = d(f, this._events, A, g, {
						context: t,
			...
			A.exports = g.extend({}, g.Events, {
				...
				popOn: function(A) {
					if (!this.isPop) switch (this.isPop = !0, this.$el.css({
						...
						backgroundImage: 'url("static/info' + A + '.png")'
					}), A) {
					case 1:
					...
					createParticles: function(A, g, t, e, C, I) {
						...
						for (var r = 0; r < g; r++) {
							var s = r % a,
								...
								E = Math.cos(l) * p,
								f = Math.sin(l) * p;
							c.position.set(E * I.x, d * I.y, f * I.z);
	...
	37: function(A, g) {
		THREE.SpriteCanvasMaterial = function(A) {
			...
			function g() {
				pA.setRGB(0, 0, 0), EA.setRGB(0, 0, 0), fA.setRGB(0, 0, 0);
				for (var A = 0, g = v.length; A < g; A++) {
					var t = v[A],
						e = t.color;
					t instanceof THREE.AmbientLight ? pA.add(e) : t instanceof THREE.DirectionalLight ? EA.add(e) : t instanceof THREE.PointLight && fA.add(e)
				}
			}
			function t(A, g, t) {
					...
					if (null !== i) {
						...
						if (void 0 !== n && n.version === i.version || (n = a(i), hA[i.id] = n), void 0 !== n.canvas) {
							...
							var o = i.image,
								...
								d = e / l,
								f = C / u;
							_.save(), _.translate(A.x, A.y), 0 !== t.rotation && _.rotate(t.rotation), _.translate(-e / 2, -C / 2), _.scale(d, f), _.translate(-r, -s), _.fillRect(r, s, l, u), _.restore()
						}
					} else E(t.color.getStyle()), _.save(), _.translate(A.x, A.y), 0 !== t.rotation && _.rotate(t.rotation), _.scale(e, -C), _.fillRect(-.5, -.5, 1, 1), _.restore()
				} else t instanceof THREE.SpriteCanvasMaterial && (p(t.color.getStyle()), E(t.color.getStyle()), _.save(), _.translate(A.x, A.y), 0 !== t.rotation && _.rotate(t.rotation), _.scale(e, C), t.program(_), _.restore())
			}
			function C(A, g, t, e) {
				...
				} else e instanceof THREE.LineDashedMaterial && (l(e.linewidth), u(e.linecap), d(e.linejoin), p(e.color.getStyle()), f([e.dashSize, e.gapSize]), _.stroke(), dA.expandByScalar(2 * e.linewidth), f([]))
			}
			...
			function r(A, g, t, e, C, I, i, n, o, r, s, c, h) {
				var l = hA[h.id];
				if (void 0 !== l && l.version === h.version || (l = a(h), hA[h.id] = l), void 0 === l.canvas) return E("rgba( 0, 0, 0, 1)"), void _.fill();
				E(l.canvas);
				var u, d, p, f, w, m, v, V, M = h.offset.x / h.repeat.x,
					x = h.offset.y / h.repeat.y,
					R = h.image.width * h.repeat.x,
					B = h.image.height * h.repeat.y;
				i = (i + M) * R, n = (n + x) * B, o = (o + M) * R, r = (r + x) * B, s = (s + M) * R, c = (c + x) * B, t -= A, e -= g, C -= A, I -= g, o -= i, r -= n, s -= i, c -= n, v = o * c - s * r, 0 !== v && (V = 1 / v, u = (c * t - r * C) * V, d = (c * e - r * I) * V, p = (o * C - s * t) * V, f = (o * I - s * e) * V, w = A - u * i - p * n, m = g - d * i - f * n, _.save(), _.transform(u, d, p, f, w, m), _.fill(), _.restore())
			}
			function s(A, g, t) {
				var e, C = g.x - A.x,
					I = g.y - A.y,
			...
			void 0 === _.setLineDash && (_.setLineDash = function() {}), this.domElement = P, this.autoClear = !0, this.sortObjects = !0, this.sortElements = !0, this.info = {
				...
				uA.isEmpty() === !1 && (uA.intersect(lA), uA.expandByScalar(2), uA.min.x = uA.min.x + N, uA.min.y = -uA.min.y + W, uA.max.x = uA.max.x + N, uA.max.y = -uA.max.y + W, AA < 1 && _.clearRect(0 | uA.min.x, 0 | uA.max.y, uA.max.x - uA.min.x | 0, uA.min.y - uA.max.y | 0), AA > 0 && (h(THREE.NormalBlending), c(1), E("rgba(" + Math.floor(255 * $.r) + "," + Math.floor(255 * $.g) + "," + Math.floor(255 * $.b) + "," + AA + ")"), _.fillRect(0 | uA.min.x, 0 | uA.max.y, uA.max.x - uA.min.x | 0, uA.min.y - uA.max.y | 0)), uA.makeEmpty())
	...
	38: function(A, g) {
		THREE.RenderableObject = function() {
			...
			var U = function() {
					function A(A) {
						E = A, f = E.material, m.getNormalMatrix(E.matrixWorld), u.length = 0, d.length = 0
					}
					function C(A) {
						var g = A.position,
							t = A.positionWorld,
					...
					function c(A, g) {
						var t = w[A],
							C = w[g];
						h = e(), h.id = E.id, h.v1.copy(t), h.v2.copy(C), h.z = (t.positionScreen.z + C.positionScreen.z) / 2, h.renderOrder = E.renderOrder, h.material = E.material, y.elements.push(h)
					}
					function l(A, g, e) {
						var C = w[A],
							I = w[g],
							i = w[e];
						if (o(C, I, i) !== !1 && (f.side === THREE.DoubleSide || r(C, I, i) === !0)) {
							s = t(), s.id = E.id, s.v1.copy(C), s.v2.copy(I), s.v3.copy(i), s.z = (C.positionScreen.z + I.positionScreen.z + i.positionScreen.z) / 3, s.renderOrder = E.renderOrder, s.normalModel.fromArray(u, 3 * A), s.normalModel.applyMatrix3(m).normalize();
							for (var n = 0; n < 3; n++) {
							...
							s.vertexNormalsLength = 3, s.material = E.material, y.elements.push(s)
						}
					}
					var u = [],
						d = [],
				...
				P = new U;
			this.projectScene = function(a, E, f, m) {
				function v(g) {
				...
				c = 0, l = 0, d = 0, y.elements.length = 0, a.autoUpdate === !0 && a.updateMatrixWorld(), null === E.parent && E.updateMatrixWorld(), H.copy(E.matrixWorldInverse.getInverse(E.matrixWorld)), G.multiplyMatrices(E.projectionMatrix, H), k.setFromMatrix(G), o = 0, y.objects.length = 0, y.lights.length = 0, a.traverseVisible(function(A) {
				...
				for (var V = 0, M = y.objects.length; V < M; V++) {
					...
					if (P.setObject(x), p = x.matrixWorld, r = 0, x instanceof THREE.Mesh) {
							...
							for (var aA = 0, rA = L.length; aA < rA; aA++) {
								...
								if (K = X === !0 ? _.materials[sA.materialIndex] : x.material, void 0 !== K) {
									...
									if (P.checkTriangleVisibility(hA, lA, uA) !== !1) {
										...
										for (var pA = sA.vertexNormals, EA = 0, fA = Math.min(pA.length, 3); EA < fA; EA++) {
											var wA = s.vertexNormalsModel[EA];
											wA.copy(pA[EA]), dA !== !1 || cA !== THREE.BackSide && cA !== THREE.DoubleSide || wA.negate(), wA.applyMatrix3(S).normalize()
										}
						...
						var MA = 1 / T.w;
						T.z *= MA, T.z >= -1 && T.z <= 1 && (u = C(), u.id = x.id, u.x = T.x * MA, u.y = T.y * MA, u.z = T.z, u.renderOrder = x.renderOrder, u.object = x, u.rotation = x.rotation, u.scale.x = x.scale.x * Math.abs(u.x - (T.x + E.projectionMatrix.elements[0]) / (T.w + E.projectionMatrix.elements[12])), u.scale.y = x.scale.y * Math.abs(u.y - (T.y + E.projectionMatrix.elements[5]) / (T.w + E.projectionMatrix.elements[13])), u.material = x.material, y.elements.push(u))
					}
				}
